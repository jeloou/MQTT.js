...
function MqttClient(streamBuilder, options) {
  ...
  // Makes the client compatible with the old way to disable reconnection
  this.options.reconnect = this.options.reconnectPeriod > 0? true : false;
...
MqttClient.prototype._setupReconnect = function() {
  ...
  if (!that.disconnecting && !that.reconnectTimer && that.options.reconnect) {
    if (that.options.reconnectAttempts !== Infinity && that.attempts++ == that.options.reconnectAttempts) {
      that.emit('reconnect_failed');
      return;
    }
    
    that.reconnectTimer = setInterval(function () {
      that._reconnect();
      that.emit('reconnect_attempt');
